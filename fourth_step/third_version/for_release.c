...
int isRed(int thread_num) {
    return 64 * thread_num;
}

int isBlue(int thread_num) {
    return 64 * thread_num + 1;
}
...
void processAlgorithm(int Nx, int Ny, int Nt, int Sx, int Sy, double tao2Hx2, double tao2Hy2, double tao, double* U0, double* U1, double* P) {
    ...
    int *syncFlags = (int*)calloc(omp_get_num_threads() * 64, sizeof(int));

#pragma omp parallel proc_bind(close)
    {
        for (int n = 0; n < Nt; n += 6) {
            int shift = omp_get_thread_num() * (Ny / omp_get_num_threads());

            int startFlag = (omp_get_thread_num() == 0 ? 0 : 1);
            int finishFlag = (omp_get_thread_num() == omp_get_num_threads() - 1 ? 0 : 1);

            int value = -1;

            if (omp_get_thread_num() != 0) {
#pragma omp atomic read
                value = syncFlags[isBlue(omp_get_thread_num() - 1)];
                while (value < n) {
#pragma omp atomic read
                    value = syncFlags[isBlue(omp_get_thread_num() - 1)];
                }
            ...
            double tao2functionF5 = tao * tao * functionF(n + 5, Nx, Ny);
            double tao2functionF6 = tao * tao * functionF(n + 6, Nx, Ny);

            for (int i = 2 - 7 * startFlag; i < 5 + 2 * (1 - startFlag); i++) {
                processLine(Nx, Ny, Nt, Sx, Sy, i + shift, tao2Hx2, tao2Hy2, tao2functionF1, U0, U1, P);
            }

            for (int i = 2 - 6 * startFlag; i < 4 + 2 * (1 - startFlag); i++) {
                processLine(Nx, Ny, Nt, Sx, Sy, i + shift, tao2Hx2, tao2Hy2, tao2functionF1, U1, U0, P);
            }

            for (int i = 2 - 5 * startFlag; i < 3 + 2 * (1 - startFlag); i++) {
                processLine(Nx, Ny, Nt, Sx, Sy, i + shift, tao2Hx2, tao2Hy2, tao2functionF1, U0, U1, P);
            }

            for (int i = 2 - 4 * startFlag; i < 2 + 2 * (1 - startFlag); i++) {
                processLine(Nx, Ny, Nt, Sx, Sy, i + shift, tao2Hx2, tao2Hy2, tao2functionF1, U1, U0, P);
            }

            for (int i = 2 - 3 * startFlag; i < 1 + 2 * (1 - startFlag); i++) {
                processLine(Nx, Ny, Nt, Sx, Sy, i + shift, tao2Hx2, tao2Hy2, tao2functionF1, U0, U1, P);
            }

            int last_element = shift + Ny / omp_get_num_threads();

            for (int i = shift + 7 - 2 * startFlag; i < last_element - 1 - 2 * finishFlag; i++) {
...
#pragma omp atomic write
            syncFlags[isRed(omp_get_thread_num())] = n + 6;

            if (omp_get_thread_num() != omp_get_num_threads() - 1) {
#pragma omp atomic read
                value = syncFlags[isRed(omp_get_thread_num() + 1)];
                while (value < n) {
#pragma omp atomic read
                    value = syncFlags[isRed(omp_get_thread_num() + 1)];
                }
            ...
            for (int i = 0; i < 1 + finishFlag; i++) {
                processLine(Nx, Ny, Nt, Sx, Sy, last_element - 4 - 2 * finishFlag + i, tao2Hx2, tao2Hy2,
                            tao2functionF6, U1, U0, P);
            }

            for (int i = 0; i < 2 + 2 * finishFlag; i++) {
                processLine(Nx, Ny, Nt, Sx, Sy, last_element - 5 - 2 * finishFlag + i, tao2Hx2, tao2Hy2,
                            tao2functionF6, U0, U1, P);
            }

            for (int i = 0; i < 3 + 3 * finishFlag; i++) {
                processLine(Nx, Ny, Nt, Sx, Sy, last_element - 6 - 2 * finishFlag + i, tao2Hx2, tao2Hy2,
                            tao2functionF6, U1, U0, P);
            }

            for (int i = 0; i < 4 + 4 * finishFlag; i++) {
                processLine(Nx, Ny, Nt, Sx, Sy, last_element - 7 - 2 * finishFlag + i, tao2Hx2, tao2Hy2,
                            tao2functionF6, U0, U1, P);
            }

            for (int i = 0; i < 5 + 5 * finishFlag; i++) {
                processLine(Nx, Ny, Nt, Sx, Sy, last_element - 8 - 2 * finishFlag + i, tao2Hx2, tao2Hy2,
                            tao2functionF6, U1, U0, P);
            }

#pragma omp atomic write
            syncFlags[isBlue(omp_get_thread_num())] = n + 6;
