...
void algorithmStepVector(int Nx, int Ny, double tao2Hx2, double tao2Hy2, double* pU0, double* pU1, double* pP) {
    ...
    __m256d leftU1 = _mm256_load_pd(pU1 - 1 - 4);
    __m256d centerU1 = _mm256_load_pd(pU1 - 1);
    __m256d rightU1 = _mm256_load_pd(pU1 - 1 + 4);

    __m256d vecLeft = _mm256_shuffle_pd(leftU1, centerU1, 15);

    __m256d U1ijV = _mm256_loadu_pd(pU1);
    __m256d U1ijp1V = _mm256_loadu_pd(pU1 + 1);
    __m256d U1ijm1V = _mm256_loadu_pd(pU1 - 1);
    __m256d U1ip1jV = _mm256_loadu_pd(pU1 + Nx);
    __m256d U1im1jV = _mm256_loadu_pd(pU1 - Nx);
...
//    _mm_free(P);
    ...
    double* U;
    U = (double*)_mm_malloc(100 * sizeof (double), 0b00000001);
